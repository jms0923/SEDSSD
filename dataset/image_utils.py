...
from dataset.box_utils import compute_iou
...
class ImageVisualizer(object):
    ...
    def save_image_cv(self, image, boxes, labels, scores, name):
        ...
        for b, l, s in zip(boxes, labels, scores):
            ...
            class_name = self.idx_to_name[l]

            xmin, ymin, xmax, ymax = list(map(int, b))
            score = '{:.4f}'.format(s)
            color = self.class_colors[l]
    ...
    def cropBoxes(self, image, boxes, name, size):
        croppedBoxes = None
        checkedOriginBoxes = []
        for idx, box in enumerate(boxes):
            ...
            if ymax - ymin < 1 or xmax - xmin < 1:
                ...
                checkedOriginBoxes.append(box)
            tmpImg = image.copy()
            box = tmpImg[ymin:ymax, xmin:xmax]

            box = cv2.resize(box, dsize=(size, size), interpolation=cv2.INTER_AREA)
            box = np.reshape(box, (1,)+box.shape)
            if croppedBoxes is not None:
                croppedBoxes = np.concatenate((croppedBoxes, box), axis=0)
            else:
                croppedBoxes = box

        return croppedBoxes, checkedOriginBoxes
