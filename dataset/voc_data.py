...
from dataset.box_utils import compute_target
from dataset.image_utils import random_patching, horizontal_flip
...
class VOCDataset:
    ...
    def generate(self, subset=None):
        ...
        for index in range(len(indices)):
            ...
            img = np.array(img.resize((self.width, self.height)), dtype=np.float32)
...
def create_batch_generator(
    ...
    voc = VOCDataset(
        root_dir, year, default_boxes, new_size, labels, num_examples, augmentation
    )
